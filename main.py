...
def cli_parser():
 ...
 |_|  \__,_|_|  |_| |_| |_|_|\__,_|_| |_|\__,_|  |_(_)___/ 
    ...
    general_options.add_argument('--intrigue','-i',help="Find what technologies are being used",dest="intrigue",required=False,default=False)
    ...
    eyewitness_options = parser.add_argument_group('Massdns Options')
    ...
    if args.masscan:
        if os.geteuid() != 0:
            parser.print_help()
            print("[!] Error : In order to use masscan you need to launch the script with root privileges")
            sys.exit()
...
if __name__ == '__main__':
    ...
    if args.crt:
        exec_crt_sh(args)
    s1_data = agregate_data_stage1(args)
